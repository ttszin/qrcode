...
class Sensors:
	...
	def getRefFrame(self, final_img, tag):
		...
		for cnt in contours:
			...
			try:
				success, rot_vec, trans_vec = cv2.solvePnP(self.model_points, self.image_points, self.camera_matrix, self.dist_coeffs, flags=cv2.SOLVEPNP_ITERATIVE)
				if success:
					axis_len = 100.0
					points = np.float32([[axis_len, 0, 0], [0, axis_len, 0], [0, 0, axis_len], [0, 0, 0]]).reshape(-1, 3)
					axisPoints, _ = cv2.projectPoints(points, rot_vec, trans_vec, self.camera_matrix, self.dist_coeffs)
			except:
				pass
		
		return rot_vec, trans_vec, success
	...
	def getRefFrameSMarks(self):
		# process the final image to get frames of reference
		R1, T1, success1 = self.getRefFrame(self.bad_mark, "Bad Marker")
		R2, T2, success2 = self.getRefFrame(self.good_mark, "Good Marker")
